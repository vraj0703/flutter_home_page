...
class TestimonialSection implements GameSection {
  final TestimonialPageComponent pageComponent;
  Vector2 screenSize;
  ...
  TestimonialSection({required this.pageComponent, required this.screenSize});
  ...
  Future<void> enter(ScrollSystem scrollSystem) async {
    _scrollProgress = 0.0;
    pageComponent.opacity = 0.0;
    pageComponent.position = screenSize / 2;
    pageComponent.size = screenSize;
  }
  ...
  void setScrollOffset(double offset) {
    ...
    if (_scrollProgress < _fadeInEnd) {
      ...
      pageComponent.opacity = t;
    }
    // Hold Visible
    else if (_scrollProgress < _fadeOutStart) {
      pageComponent.opacity = 1.0;
    }
    // Fade Out
    else {
      final t =
  ...
  void onResize(Vector2 newSize) {
    screenSize = newSize;
    pageComponent.position = newSize / 2;
    pageComponent.size = newSize;
  }
